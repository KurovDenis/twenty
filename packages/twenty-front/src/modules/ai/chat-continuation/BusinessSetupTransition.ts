import {
  ...
  BusinessSetupStatus,
} from '~/modules/business-setup/hooks/useSetNextBusinessSetupStatus';
...
export class BusinessSetupTransition {
  ...
  static analyzeTransition(
    ...
    const unmetConditions = rule.conditions.filter(
  ...
  static getProgress(currentStep: BusinessSetupStatus): number {
    ...
    return currentIndex >= 0
      ? Math.round((currentIndex / (allSteps.length - 1)) * 100)
      : 0;
  }
}

...
} from '../hooks/useBusinessSetupSubscriptions';
 ...
 */
export interface BusinessSetupEventContextValue
  extends BusinessSetupEventState,
    BusinessSetupEventActions {}

/**
 * Create the context
 */
const BusinessSetupEventContext =
  createContext<BusinessSetupEventContextValue | null>(null);

/**
 * Hook to use the Business Setup Event Context
 */
export const useBusinessSetupEventContext =
  (): BusinessSetupEventContextValue => {
    const context = useContext(BusinessSetupEventContext);
    if (!context) {
      throw new Error(
        'useBusinessSetupEventContext must be used within a BusinessSetupEventProvider',
      );
    }
    return context;
  };
 ...
 */
export const BusinessSetupEventProvider: React.FC<
  BusinessSetupEventProviderProps
> = ({ children, enableLogging = false }) => {
  // Local state
  const [showWelcomePopup, setShowWelcomePopup] = useState(false);
  const [isWelcomeMessageProcessed, setIsWelcomeMessageProcessed] =
    useState(false);
  ...
  useEffect(() => {
    if (enableLogging && subscriptions.lastOnboardingChange) {
      console.log(
        ...
        subscriptions.lastOnboardingChange,
      );
    }
  ...
  useEffect(() => {
    if (subscriptions.hasErrors) {
      const error =
        subscriptions.businessSetupEvents.connectionError ||
  ...
  useEffect(() => {
    ...
    if (
      ...
      setProcessedEventIds(
        (prev) => new Set([...prev, `welcome-${latestWelcomeEvent.threadId}`]),
      );
  ...
  const previousOnboardingStatus =
    subscriptions.onboardingStatus.previousStatus;
  const activeWelcomeThread =
    subscriptions.lastWelcomeChatEvent?.threadId || null;
  const lastWelcomeMessage =
    subscriptions.lastWelcomeChatEvent?.status === 'CHAT_CREATED'

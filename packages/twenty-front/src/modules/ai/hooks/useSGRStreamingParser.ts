...
export const useSGRStreamingParser = (
  ...
  // Основное состояние
  const [visualizationState, setVisualizationState] = useState<SGRVisualizationState>({
    status: 'idle',
    threadId: null,
    stepId: null,
  });
   ...
   */
  const parsePartialJson = useCallback((jsonString: string): Record<string, unknown> => {
   ...
   */
  const handleStreamingEvent = useCallback((event: SGRStreamEvent) => {
    ...
    setVisualizationState(prevState => {
      ...
      switch (type) {
        ...
        case 'SGR_JSON_TOKEN_CHUNK':
          ...
          const updatedJson = newState.jsonStreaming.rawJson + (payload.token || '');
        ...
        case 'SGR_JSON_STREAM_END':
          const finalJson = payload.fullJson || newState.jsonStreaming?.rawJson || '';
  ...
  const getStreamingMetrics = useCallback(() => {
    const { jsonStreaming, startTime, endTime, totalProcessingTime } = visualizationState;
    
    return {
      isActive: visualizationState.status !== 'idle' && visualizationState.status !== 'completed',
      totalTokens: jsonStreaming?.totalTokens || 0,
      streamingSpeed: jsonStreaming?.streamingSpeed || 0,
      processingTime: totalProcessingTime || (startTime ? Date.now() - startTime.getTime() : 0),
  ...
  return {
    ...
    isStreaming: visualizationState.status !== 'idle' && visualizationState.status !== 'completed',

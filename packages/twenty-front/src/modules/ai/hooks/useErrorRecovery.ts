...
export const useErrorRecovery = () => {
    ...
    async <T>(
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          console.warn(`Operation failed on attempt ${attempt}/${maxRetries}`, {
          ...
          if (!classification.recoverable) {
            throw new Error(`Non-recoverable error: ${lastError.message}`);
          }
   ...
   */
  const executeWithClassification = useCallback(
    async <T>(operation: () => Promise<T>, maxRetries = 3): Promise<T> => {
      try {
   ...
   */
  const executeWithCustomRetry = useCallback(
    async <T>(
      operation: () => Promise<T>,
      retryConfig: {
        maxRetries?: number;

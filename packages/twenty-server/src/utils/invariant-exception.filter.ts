...
import { InvariantError, invariant, error as logError } from 'twenty-shared/utils';
...
@Injectable()
export class InvariantExceptionFilter implements ExceptionFilter<InvariantError> {
  ...
  private mapErrorCodeToHttpStatus(code: number): HttpStatus {
    ...
    if (code >= 4000 && code < 5000) {
      ...
      return code < 4500 ? HttpStatus.CONFLICT : HttpStatus.UNPROCESSABLE_ENTITY;
    }
    ...
    descriptor.value = async function (...args: any[]) {
      try {
        ...
        if (error instanceof InvariantError) {
          logError(`Invariant error in ${target.constructor.name}.${propertyName}:`, error.message);
        }

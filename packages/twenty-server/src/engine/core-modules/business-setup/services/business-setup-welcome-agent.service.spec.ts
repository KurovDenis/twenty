...
describe('BusinessSetupWelcomeAgentService - Avito Integration', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(AgentEntity, 'core'), useValue: mockAgentRepository },
      ],
    }).compile();
  ...
  describe('extractCredentialsFromMessage', () => {
    ...
    it('should handle spaces around separators', () => {
      const message = "CLIENT_ID  :  spaced_id\nCLIENT_SECRET  =  spaced_secret";
  ...
  describe('parseValidationResult', () => {
    it('should return true for successful validation response', () => {
      const successResponse = 'HTTP request successful, access_token received, status: 200';
    ...
    it('should return false for error response', () => {
      const errorResponse = 'HTTP request failed, status: 401, error: invalid credentials';
  ...
  describe('processUserMessage', () => {
    ...
    it('should process valid credentials and trigger validation', async () => {
      ...
      expect(mockAgentExecutionService.executeAgent).toHaveBeenCalledWith(
        expect.objectContaining({
          schema: {},
          userPrompt: expect.stringContaining('CLIENT_ID: R3cTDMk9rEJ2lh5A9_QF')
        })
      );
    ...
    it('should send retry message for invalid credentials format', async () => {
      ...
      expect(mockAgentChatService.addMessage).toHaveBeenCalledWith({
        ...
        content: expect.stringContaining('Не удалось найти CLIENT_ID и CLIENT_SECRET'),
        fileIds: []
      });
    ...
    it('should handle agent execution errors gracefully', async () => {
      ...
      mockAgentExecutionService.executeAgent.mockRejectedValue(new Error('Agent execution failed'));

      // Should not throw
      await expect(service.processUserMessage(threadId, message, workspaceId, userId)).resolves.not.toThrow();
    });
  });
  ...
  describe('storeAvitoCredentials', () => {
    it('should store both CLIENT_ID and CLIENT_SECRET', async () => {
      ...
      await (service as any).storeAvitoCredentials(workspaceId, userId, credentials);
  ...
  describe('transitionToBusinessAnalysis', () => {
    it('should update business setup step statuses correctly', async () => {
      ...
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('business-setup.step-transition', {
        userId: 'user-123',
        workspaceId: 'workspace-123',
        fromStep: 'WELCOME',
  ...
  describe('getAvitoWelcomePrompt', () => {
    it('should generate Russian welcome prompt with user and workspace info', async () => {
      const prompt = await (service as any).getAvitoWelcomePrompt(mockUser, mockWorkspace);
    ...
    it('should handle user without firstName', async () => {
      const userWithoutName = { ...mockUser, firstName: null } as unknown as User;
      
      const prompt = await (service as any).getAvitoWelcomePrompt(userWithoutName, mockWorkspace);
  ...
  describe('handleValidationResponse', () => {
    ...
    it('should not store credentials on failed validation', async () => {
      const agentResponse = {
        result: { response: 'HTTP request failed, status: 401, invalid credentials' }
      };
      const credentials: CredentialsExtractionResult = {
        clientId: 'test_id',
        clientSecret: 'test_secret',

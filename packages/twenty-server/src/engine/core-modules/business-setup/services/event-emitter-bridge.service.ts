...
export class EventEmitterBridgeService implements OnModuleDestroy {
  ...
  private readonly tokenThrottleMs =
    DEFAULT_SGR_STREAMING_CONFIG.tokenThrottleMs;
  private readonly maxTokensPerBatch =
    DEFAULT_SGR_STREAMING_CONFIG.maxTokensPerChunk;
  ...
  async handleSGRStreamingEvent(payload: SGRStreamEvent): Promise<void> {
    ...
    try {
      ...
      const subscriptionPayload =
        this.formatSGRStreamingEvent(sanitizedPayload);
  ...
  async handleSupervisorSGREvent(payload: SupervisorSGREvent): Promise<void> {
    ...
    try {
      ...
      // Форматирование для подписки
      const subscriptionPayload =
        this.formatSupervisorSGREvent(sanitizedPayload);
      ...
      this.logger.debug(`Successfully published Supervisor SGR event`);
    } catch (error) {
  ...
  async handleSGRTokenChunk(payload: {
    token: string;
    threadId: string;
    stepId: string;
  }): Promise<void> {
  ...
  private sanitizeSGRPayload(payload: SGRStreamEvent): SGRStreamEvent {
    if (
      payload.type === SGRStreamEventType.TOOL_CALL_PENDING &&
      payload.payload
    ) {
      return {
        ...payload,
    ...
    if (
      payload.type === SGRStreamEventType.JSON_STREAM_END &&
      payload.payload?.fullJson
    ) {
      return {
        ...payload,
        payload: {
   ...
   */
  private sanitizeSupervisorSGRPayload(
    payload: SupervisorSGREvent,
  ): SupervisorSGREvent {
    return {
      ...payload,
  ...
  private removeSensitiveData(data: any): any {
    ...
    const sensitiveKeys =
      DEFAULT_SGR_STREAMING_CONFIG.sanitizationConfig.sensitiveKeys;
   ...
   */
  private formatSGRStreamingEvent(
    payload: SGRStreamEvent,
  ): SubscriptionEventPayload {
   ...
   */
  private formatSupervisorSGREvent(
    payload: SupervisorSGREvent,
  ): SubscriptionEventPayload {
    return {
      id: this.generateEventId(),
   ...
   */
  private mapSGREventTypeToBusinessSetupType(
    sgrType: SGRStreamEventType,
  ): BusinessSetupEventType {
    const mapping: Record<SGRStreamEventType, BusinessSetupEventType> = {
      [SGRStreamEventType.PROCESS_START]:
        BusinessSetupEventType.SGR_STREAMING_START,
      [SGRStreamEventType.JSON_STREAM_START]:
        BusinessSetupEventType.SGR_JSON_STREAM_START,
      [SGRStreamEventType.JSON_TOKEN_CHUNK]:
        BusinessSetupEventType.SGR_JSON_TOKEN_CHUNK,
      [SGRStreamEventType.JSON_STREAM_END]:
        BusinessSetupEventType.SGR_JSON_STREAM_END,
      [SGRStreamEventType.TOOL_CALL_PENDING]:
        BusinessSetupEventType.SGR_TOOL_CALL_PENDING,
      [SGRStreamEventType.PROCESS_END]:
        BusinessSetupEventType.SGR_STREAMING_END,
      [SGRStreamEventType.PROCESS_ERROR]:
  ...
  private updateErrorMetrics(): void {
    this.sgrMetrics.errorRate = this.sgrMetrics.errorRate * 0.9 + 0.1; // Exponential moving average
  }

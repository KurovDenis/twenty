...
export class EventEmitterBridgeService {
  ...
  async handleOnboardingStatusChange(payload: OnboardingStatusChangedEvent): Promise<void> {
    try {
      this.logger.log(`Processing onboarding status change: ${payload.status} for user ${payload.userId}`);
      ...
      await this.publishToSubscriptionWithRetry('businessSetupEvents', subscriptionPayload);
      
      this.logger.log(`Successfully published onboarding status change to subscription`);
    } catch (error) {
  ...
  async handleWelcomeChatCreated(payload: WelcomeChatCreatedEvent): Promise<void> {
    try {
      this.logger.log(`Processing welcome chat created event for user ${payload.userId}, thread ${payload.threadId}`);
      ...
      await this.publishToSubscriptionWithRetry('businessSetupEvents', subscriptionPayload);
      await this.publishToSubscriptionWithRetry('aiAgentEvents', subscriptionPayload);
      
      this.logger.log(`Successfully published welcome chat created to subscriptions`);
    } catch (error) {
  ...
  async handleWelcomeChatCreationFailed(payload: WelcomeChatCreationFailedEvent): Promise<void> {
    try {
      this.logger.log(`Processing welcome chat creation failed event for user ${payload.userId}`);
      ...
      this.logger.log(`Successfully published welcome chat creation failed to subscriptions`);
    } catch (error) {
      this.logger.error('Failed to publish welcome chat creation failed:', error);
    }
  }
   ...
   */
  private async publishToSubscriptionWithRetry(channel: string, payload: any): Promise<void> {
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        await this.pubSub.publish(channel, payload);
        return; // Success
      } catch (error) {
        this.logger.warn(`Attempt ${attempt} failed to publish to ${channel}:`, error);
        
        if (attempt === this.maxRetries) {
          this.logger.error(`All ${this.maxRetries} attempts failed to publish to ${channel}`);
          throw error;
        }

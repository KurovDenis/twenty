...
export class BusinessSetupTransitionService {
  ...
  async transitionToNextStep(
    ...
    try {
      this.logger.log(
        `Transitioning from ${input.fromStep} to ${input.toStep} for user ${userId}`,
      );

      // Валидируем переход
      if (!this.isValidTransition(input.fromStep, input.toStep)) {
        throw new Error(
          `Invalid transition from ${input.fromStep} to ${input.toStep}`,
        );
      }
      ...
      this.logger.log(
        `Successfully transitioned to ${input.toStep} for user ${userId}`,
      );
  ...
  async getBusinessSetupProgress(
    ...
    try {
      const currentStep =
        await this.businessSetupService.getBusinessSetupStatus(
          { id: userId } as any,

...
/**
 * Main SGR (Schema-Guided Reasoning) execution service for Avito Welcome Agent
 * 
...
`;
    ...
    try {
      ...
      await this.agentChatService.addMessage({
        ...
        content: '❌ Произошла ошибка при обработке запроса. Попробуйте еще раз или обратитесь в поддержку.',
        fileIds: []
      });
    }
  }
  ...
  async *processWelcomeMessageWithStreaming(
    ...
    this.logger.log(`Processing welcome message with STREAMING SGR for user ${userId}`);
...
`;
   ...
   */
  private async executeSGRWorkflow(params: SGRExecutionParams): Promise<SGRExecutionResult> {
    this.logger.log(`Starting SGR workflow with max ${params.maxSteps || 20} steps`);
    ...
    for (let stepNumber = 1; stepNumber <= maxSteps; stepNumber++) {
      try {
        ...
        // Log progress
        this.logger.log(`Step ${stepNumber} completed: ${stepResult.function.tool} -> ${toolResult.success ? 'success' : 'failed'}`);
    ...
    return {
      ...
      credentials_stored: false,
      next_stage: 'error_retry',
      summary: 'Превышено максимальное количество шагов обработки. Попробуйте еще раз.',
      steps_executed: stepsExecuted,
  ...
  private async *executeSGRWorkflowWithStreaming(
    ...
    this.logger.log(`Starting STREAMING SGR workflow with max ${params.maxSteps || 20} steps`);
    ...
    for (let stepNumber = 1; stepNumber <= maxSteps; stepNumber++) {
      try {
        this.logger.log(`Executing STREAMING SGR step ${stepNumber}/${maxSteps}`);

        // STREAM: Start thinking step
        const thinkingStep: SGRThinkingStep = {
          stepNumber,
          currentState: `Анализирую шаг ${stepNumber} из ${maxSteps}...`,
          plannedSteps: ['Получение структурированного решения от AI', 'Выполнение выбранного инструмента', 'Анализ результата'],
        ...
        // Log progress
        this.logger.log(`STREAMING Step ${stepNumber} completed: ${stepResult.function.tool} -> ${toolResult.success ? 'success' : 'failed'}`);
    ...
    // Workflow exceeded maximum steps
    this.logger.warn(`STREAMING SGR workflow exceeded maximum steps (${maxSteps})`);
    yield {
      type: 'final_response',
      content: 'Превышено максимальное количество шагов обработки. Попробуйте еще раз.',
      completed: true
    };
  }
   ...
   */
  private async executeReasoningStep(context: WelcomeExecutionContext & {
    conversationLog: any[]
   ...
   */
  private async executeReasoningStepWithStreaming(context: WelcomeExecutionContext & {
    conversationLog: any[]
  }): Promise<SGRStepResult> {
    ...
    try {
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(
            new SGRStreamingException(
              ...
              { stepNumber: context.stepNumber, timeoutMs: DEFAULT_SGR_THINKING_CONFIG.stepTimeoutMs }
            )
      ...
      this.logger.log(`STREAMING reasoning step ${context.stepNumber} result:`, {
        current_state: stepResult.current_state.substring(0, 100) + '...',
        planned_steps: stepResult.plan_remaining_steps.length,
        selected_tool: stepResult.function.tool,
        task_completed: stepResult.task_completed
      ...
      this.logger.error(`STREAMING reasoning step ${context.stepNumber} failed:`, error);
  ...
  private async transitionToNextStage(
    ...
    try {
      if (nextStage === 'business_analysis') {
        ...
        this.logger.log(`Successfully transitioned user ${userId} from WELCOME to BUSINESS_ANALYSIS`);
      }
    } catch (error) {
   ...
   */
  async getWelcomeStatus(userId: string, workspaceId: string): Promise<{
